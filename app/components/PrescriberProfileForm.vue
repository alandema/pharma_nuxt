...
import { GENDER_OPTIONS, TITLE_OPTIONS } from "#shared/utils/commonOptions";
...
const handleSubmit = async () => {
  try {
    ...
    if (
      canEditOwnIdentity.value &&
      hasRequiredValue(payload.email) &&
    ...
    if (
      hasRequiredValue(payload.birth_date) &&
      !isValidBirthDate(String(payload.birth_date))
    ) {
    ...
    if (
      hasRequiredValue(payload.zipcode) &&
      !isValidBrazilCep(payload.zipcode)
    ) {
    ...
    <form @submit.prevent="handleSubmit" class="grid-form">
      ...
      <div class="form-group">
        ...
        <input
          ...
          placeholder="Deixe em branco para não alterar (8-25, letras e números)"
        />
      </div>
      ...
      <div class="form-group">
        ...
        <select v-model="profile.title">
          <option :value="null">Sem título</option>
