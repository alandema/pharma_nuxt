...
if (!prescriberId.value) {
  throw createError({ statusCode: 400, statusMessage: 'ID de prescritor inválido.' })
}

const { data: prescriber, refresh } = await useFetch<Prescriber>(`/api/users/admin/${prescriberId.value}`)

async function toggleActive() { 
  await $fetch(`/api/users/admin/${prescriberId.value}`, { method: 'PUT', body: {} })
  await refresh() 
}
...
<template>
  ...
  <div v-if="prescriber">
    <div style="max-width: 800px; margin: 0 auto;">
      <div v-if="currentUser?.id !== prescriber.id" class="btn-group" style="margin-bottom:1rem">
